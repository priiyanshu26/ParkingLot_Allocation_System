...
//       await axios.put(`http://localhost:5174/api/vehicle/${id}`, formData, {
...
const UpdateVehicle = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      setFeedback({ type: 'success', text: 'Vehicle updated successfully!' });
      setTimeout(() => navigate('/user/dashboard'), 900);
    } catch {
      setFeedback({ type: 'danger', text: 'Update failed. Please check details and try again.' });
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="text-center mb-4">
          <i className="bi bi-pencil-square text-light" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}></i>
          <h2 className="fw-bold mt-3" style={{ fontSize: 'clamp(1.5rem, 4vw, 2.4rem)' }}>UPDATE VEHICLE</h2>
          <p className="text-secondary" style={{ fontSize: 'clamp(0.85rem, 1.8vw, 1.05rem)' }}>Modify your vehicle details below</p>
        </div>
        ...
        <div className="d-flex justify-content-between mt-4 gap-2">
          ...
          <button className="btn btn-warning py-2" style={{ fontSize: 'clamp(0.95rem, 1.8vw, 1.15rem)' }} onClick={handleUpdate}>
